...
public class LazyEntityDataModel<T> extends LazyDataModel<T> {
    ...
    public List<T> load(int first, int pageSize, String sortField, SortOrder sortOrder, Map<String, FilterMeta> filters) {
        if (this.facade != null) { // Handle data that needs to be retrieved from the data back-end of the application
            ...
            if (!this.itemList.isEmpty()) {
                System.out.println("Clase :  " + this.itemList.get(0).getClass().getName() + " | Cantidad de la grilla : " + this.itemList.size() + " | Tamano de registro : " + this.getRowCount());
            } else {
                System.out.println("Cantidad de la grilla : " + this.itemList.size() + " | Tamano de registro : " + this.getRowCount());
            }
    ...
    private List<T> filter(List<T> itemList, Map<String, FilterMeta> filters) {
